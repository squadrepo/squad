...
export const ProfileEditScreen = ({ navigation, route }) => {
  const { aboutMe, username, fullName, pfpUrl, tags, uid } =
    React.useContext(UserContext);

  //URL used to POST users info
  const url =
    "https://ca8vo445sl.execute-api.us-east-1.amazonaws.com/test/editprofile";
  const updateData = {
    uid: uid,
    username: username,
    aboutMe: aboutMe,
  ...
  function getPfpUrl() {
    if (route.params?.pfpUrl) {
      uploadPfp();
      updateData["pfpUrl"] =
        "https://squad-app-s3.s3.amazonaws.com/" +
        route.params?.pfpUrl.split("/").pop();
  ...
  return (
    <View>
      <Appbar.Header>
        ...
        <Button textDecoration="underline" onPress={save}>
          Save
        </Button>
      </Appbar.Header>
