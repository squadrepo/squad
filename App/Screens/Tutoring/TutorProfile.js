...
export const TutorProfile = ({ navigation, route}) => {
    const {tutorUid, tutorRating, totalNumRatings, numDisciples, root, subject, univ} = route.params;
    ...
    function renderAvailability(avail) {
        ...
        return days
          ...
          .map((day) => {
            const times = avail[day].map(([start, end]) => `${start.toString().slice(0,-2)}:${start.toString().slice(-2)} - ${end.toString().slice(0,-2)}:${end.toString().slice(-2)}`);
            return (
    ...
    return (
        <SafeAreaView>
            <Appbar.Header>
                <Appbar.BackAction onPress={() => navigation.navigate(root, {subject: subject, univ: univ})} />
            ...
            <ScrollView>
                ...
                <View>
                    ...
                    <Caption style={styles.caption}>Currently working with {numDisciples} student{getStandardPlural(numDisciples)}</Caption>
